#!/usr/bin/env python3
"""
Facebook Analytics Agent - Simplified approach using metrics files
"""
import json
import asyncio
...
class FacebookAnalyticsAgent:
    """Single agent that consumes Facebook metrics and produces insights"""

    def __init__(self, metrics_dir="./metrics/facebook", insights_dir="./insights/facebook", ollama_host="http://localhost:11434"):
        self.metrics_dir = Path(metrics_dir)
        self.insights_dir = Path(insights_dir)
        self.insights_dir.mkdir(parents=True, exist_ok=True)
        self.ollama_host = ollama_host
        self.name = "facebook_analytics"
    ...
    async def analyze_performance(self, focus_area: str = "comprehensive", skip_if_exists: bool = False) -> Dict[str, Any]:
        ...
        if not metrics:
            raise FileNotFoundError("No Facebook metrics found. Run ingestion first.")
    ...
    def _load_all_metrics(self) -> Dict[str, Any]:
        ...
        summary_file = self.metrics_dir / "latest_metrics_summary_facebook.json"
        if summary_file.exists():
            with open(summary_file) as f:
                metrics["summary"] = json.load(f)

        metric_types = [
            "facebook_brand_performance",
            "facebook_content_type_performance",
            "facebook_temporal_analytics",
            "facebook_top_performers",
            "facebook_worst_performers",
        ]
    ...
    def _existing_analysis_file(self, focus: str, dataset_id: str) -> Optional[Path]:
        ...
        pattern = f"ai_insights_facebook_{focus}_{safe_ds}_*.json"
    ...
    def _existing_exec_file(self, dataset_id: str) -> Optional[Path]:
        safe_ds = self._safe_dataset_id(dataset_id)
        pattern = f"facebook_executive_summary_{safe_ds}_*.json"
        files = list(self.insights_dir.glob(pattern))
        if files:
            return max(files, key=lambda f: f.stat().st_mtime)
        return None
    ...
    def _metrics_ready(self) -> bool:
        # Require latest summary plus at least one detail file
        summary_ok = (self.metrics_dir / "latest_metrics_summary_facebook.json").exists()
        any_detail = any(self.metrics_dir.glob("facebook_*_performance_*.json")) or \
                     any(self.metrics_dir.glob("facebook_temporal_analytics_*.json"))
        return summary_ok and any_detail

    def _insights_ready(self, dataset_id: Optional[str]) -> bool:
        safe_ds = self._safe_dataset_id(dataset_id)
        # Require all four focus JSONs + executive summary JSON
        focuses = [
            'comprehensive', 'performance_optimization', 'content_strategy', 'posting_optimization'
        ]
        focus_ok = all(list(self.insights_dir.glob(f"ai_insights_facebook_{f}_{safe_ds}_*.json")) for f in focuses)
        exec_ok = any(self.insights_dir.glob(f"facebook_executive_summary_{safe_ds}_*.json"))
        return focus_ok and exec_ok

    async def _comprehensive_analysis(self, metrics: Dict) -> Dict[str, Any]:
        summary = metrics.get("summary", {})
        top_performers = metrics.get("facebook_top_performers", {})
        worst_performers = metrics.get("facebook_worst_performers", {})
...
"""
    ...
    async def _performance_optimization_analysis(self, metrics: Dict) -> Dict[str, Any]:
        summary = metrics.get("summary", {})
        worst_performers = metrics.get("facebook_worst_performers", {})
        prompt = "Focus on concrete steps to improve underperforming Facebook content based on worst performers."
    ...
    async def _content_strategy_analysis(self, metrics: Dict) -> Dict[str, Any]:
        summary = metrics.get("summary", {})
        prompt = "Give content strategy recommendations for Facebook based on brand and content-type performance."
        ai_response = await self._get_ollama_response(prompt)
    ...
    async def _posting_optimization_analysis(self, metrics: Dict) -> Dict[str, Any]:
        summary = metrics.get("summary", {})
        prompt = "Recommend optimal posting times for Facebook based on temporal analytics."
        ai_response = await self._get_ollama_response(prompt)
    ...
    def _save_analysis_outputs(self, result: Dict, focus: str):
        ...
        json_file = self.insights_dir / f"ai_insights_facebook_{focus}_{safe_ds}_{ts}.json"
    ...
    def _save_analysis_markdown(self, result: Dict, focus: str):
        ...
        ds = result.get('dataset_id') or 'dataset'
        safe_ds = ''.join(c if (c.isalnum() or c in ('-','_')) else '-' for c in str(ds))
        md_file = self.insights_dir / f"ai_insights_facebook_{focus}_{safe_ds}_{ts}.md"
        with open(md_file, 'w') as f:
            f.write(f"# Facebook AI Insights - {focus.replace('_',' ').title()}\n\n")
    ...
    async def generate_executive_summary(self, skip_if_exists: bool = False) -> Dict[str, Any]:
        ...
        prompt = f"""
Generate an executive summary for Facebook marketing performance:
...
"""
    ...
    def _save_executive_outputs(self, result: Dict):
        ...
        # 1) Executive JSON
        exec_json = self.insights_dir / f"facebook_executive_summary_{safe_ds}_{ts}.json"
        with open(exec_json, 'w') as f:
            json.dump(result, f, indent=2, default=str)

        # 2) Latest dashboard JSON
        dashboard_file = self.insights_dir / "facebook_executive_dashboard.json"
        dashboard_data = {
        ...
        latest_exec = self.insights_dir / "latest_executive_summary_facebook.json"
        with open(latest_exec, 'w') as f:
            json.dump(result, f, indent=2, default=str)

        # 4) Executive markdown
        md_file = self.insights_dir / f"facebook_executive_report_{safe_ds}_{ts}.md"
        with open(md_file, 'w') as f:
            f.write("# Facebook Marketing Performance - Executive Report\n\n")
...
async def analyze_facebook_performance(focus_area: str = "comprehensive") -> Dict[str, Any]:
    agent = FacebookAnalyticsAgent()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Facebook Analytics Agent')
    ...
    async def main():
        ...
        if args.all:
            ...
            if agent._metrics_ready() and agent._insights_ready(ds):
                ...
                cmd = [
                    ...
                    '--metrics-dir', './metrics/facebook',
                    '--insights-dir', './insights/facebook',
                ]
        ...
        if args.executive:
            ...
            print("\n" + "="*50)
            print("FACEBOOK EXECUTIVE SUMMARY")
            print("="*50)
            ...
            print("\n" + "="*50)
            print(f"FACEBOOK ANALYSIS - {args.focus.upper()}")
            print("="*50)
