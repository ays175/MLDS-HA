#!/usr/bin/env python3
"""
Facebook metrics export system for agentic AI consumption
Saves to ./metrics/facebook and prefixes filenames with 'facebook_'
"""
import pandas as pd
import numpy as np
import json
...
class FacebookMetricsExporter:
    """Export Facebook metrics in AI-agent-friendly formats"""

    def __init__(self, output_dir="./metrics/facebook"):
    ...
    def export_all_metrics(self, df: pd.DataFrame, entities: Dict, dataset_id: str):
        ...
        print("📊 Generating Facebook AI-agent metrics...")
        ...
        self.save_json(overview, f"facebook_dataset_overview_{self.timestamp}.json")
        flat_overview = {
        ...
        self.save_csv([flat_overview], f"facebook_dataset_overview_{self.timestamp}.csv")

        # 1b. Per-post sampled metrics for distributions (max 5000 rows)
        try:
            sample_cols = ['facebook_insights_engagements','facebook_insights_video_views','facebook_insights_impressions','facebook_insights_video_views_average_completion']
            ...
            if n > 0:
                ...
                self.save_json({'per_post_sample': sample_out}, f"facebook_per_post_sample_{self.timestamp}.json")
        except Exception:
            pass

        # 2. Performance Metrics by Brand
        brand_metrics = self.generate_brand_performance_metrics(df)
        self.save_json(brand_metrics, f"facebook_brand_performance_{self.timestamp}.json")
        self.save_csv(brand_metrics.get('brand_summary', []), f"facebook_brand_performance_{self.timestamp}.csv")

        # 3. Content Type Performance
        content_metrics = self.generate_content_type_metrics(df)
        self.save_json(content_metrics, f"facebook_content_type_performance_{self.timestamp}.json")
        self.save_csv(content_metrics.get('content_summary', []), f"facebook_content_type_performance_{self.timestamp}.csv")

        # 4. Temporal Performance Analytics
        temporal_metrics = self.generate_temporal_metrics(df)
        self.save_json(temporal_metrics, f"facebook_temporal_analytics_{self.timestamp}.json")
        self.save_csv(temporal_metrics.get('hourly_performance', []), f"facebook_hourly_performance_{self.timestamp}.csv")
        self.save_csv(temporal_metrics.get('daily_performance', []), f"facebook_daily_performance_{self.timestamp}.csv")
        self.save_json({'hourly_performance': temporal_metrics.get('hourly_performance', [])}, f"facebook_hourly_performance_{self.timestamp}.json")
        self.save_json({'daily_performance': temporal_metrics.get('daily_performance', [])}, f"facebook_daily_performance_{self.timestamp}.json")

        # 5. Top Performers Analysis
        top_performers = self.generate_top_performers_analysis(df)
        self.save_json(top_performers, f"facebook_top_performers_{self.timestamp}.json")
        self.save_csv(top_performers.get('top_posts', []), f"facebook_top_performers_{self.timestamp}.csv")

        # 6. Worst Performers Analysis
        worst_performers = self.generate_worst_performers_analysis(df)
        self.save_json(worst_performers, f"facebook_worst_performers_{self.timestamp}.json")
        self.save_csv(worst_performers.get('worst_posts', []), f"facebook_worst_performers_{self.timestamp}.csv")
        ...
        self.save_json(ai_guide, f"facebook_ai_agent_guide_{self.timestamp}.json")
        ...
        self.save_json(consolidated, "latest_metrics_summary_facebook.json")

        print(f"✅ All Facebook metrics exported to {self.output_dir}")
        return consolidated

    def generate_dataset_overview(self, df: pd.DataFrame, dataset_id: str) -> Dict:
        df = df.copy()
        numeric_cols = ['facebook_insights_impressions', 'facebook_insights_video_views', 'facebook_insights_engagements', 'facebook_insights_reach']
        for col in numeric_cols:
            df[col] = pd.to_numeric(df.get(col, 0), errors='coerce').fillna(0)

        with pd.option_context('mode.use_inf_as_na', True):
            er_series = (df['facebook_insights_engagements'] / df['facebook_insights_impressions'] * 100).replace([pd.NA, pd.NaT], 0).fillna(0)
        cr_series = pd.to_numeric(df.get('facebook_insights_video_views_average_completion', 0), errors='coerce').fillna(0)
        ...
        overview = {
            ...
            "key_metrics": {
                "total_impressions": int(df['facebook_insights_impressions'].sum()),
                "total_engagements": int(df['facebook_insights_engagements'].sum()),
                "total_video_views": int(df['facebook_insights_video_views'].sum()),
                "average_engagement_rate": round((df['facebook_insights_engagements'].sum() / df['facebook_insights_impressions'].sum() * 100), 2) if df['facebook_insights_impressions'].sum() > 0 else 0,
                "average_completion_rate": round(float(df['facebook_insights_video_views_average_completion'].mean() or 0), 2),
                "max_engagements_post": int(df['facebook_insights_engagements'].max() or 0),
                "max_views_post": int(df['facebook_insights_video_views'].max() or 0),
                "max_impressions_post": int(df['facebook_insights_impressions'].max() or 0)
            },
            "performance_benchmarks": {
                "high_engagement_threshold": float(df['facebook_insights_engagements'].quantile(0.8)) if len(df) else 0.0,
                "high_completion_threshold": float(df['facebook_insights_video_views_average_completion'].quantile(0.8)) if len(df) else 0.0,
                "viral_threshold": float(df['facebook_insights_video_views'].quantile(0.95)) if len(df) else 0.0
            },
            "quality_metrics": {
                ...
                "completion_rate_median": round(float(cr_series.quantile(0.5) if len(cr_series) else 0.0), 2),
                "completion_rate_p75": round(float(cr_series.quantile(0.75) if len(cr_series) else 0.0), 2),
                "completion_rate_p95": round(float(cr_series.quantile(0.95) if len(cr_series) else 0.0), 2)
            }
        }
    ...
    def generate_brand_performance_metrics(self, df: pd.DataFrame) -> Dict:
        ...
        for _, row in df.iterrows():
            labels = str(row.get('facebook_post_labels_names', ''))
            brands = self._extract_brands_from_labels(labels)
            for brand in brands:
                brand_data.append({
                    'post_id': row.get('facebook_id'),
                    'brand': brand,
                    'impressions': _to_int(row.get('facebook_insights_impressions', 0)),
                    'engagements': _to_int(row.get('facebook_insights_engagements', 0)),
                    'video_views': _to_int(row.get('facebook_insights_video_views', 0)),
                    'completion_rate': _to_float(row.get('facebook_insights_video_views_average_completion', 0)),
        ...
        brand_summary.columns = ['total_posts', 'total_impressions', 'avg_impressions',
                                 'total_engagements', 'avg_engagements', 'total_views', 'avg_views', 'avg_completion_rate']
    ...
    def generate_temporal_metrics(self, df: pd.DataFrame) -> Dict:
        ...
        hourly_perf = df.groupby('hour').agg({
            ...
            'facebook_insights_video_views': 'mean',
            'facebook_insights_video_views_average_completion': 'mean',
            'facebook_id': 'count'
        }).round(2)
        ...
        hour_by_day = [
            {
                ...
                'hour': int(row['hour']),
                'avg_engagements': float(row['facebook_insights_engagements']) if pd.notna(row['facebook_insights_engagements']) else 0.0,
                'avg_views': float(row['facebook_insights_video_views']) if pd.notna(row['facebook_insights_video_views']) else 0.0,
            }
    ...
    def generate_top_performers_analysis(self, df: pd.DataFrame) -> Dict:
        df = df.copy()
        df['engagement_rate'] = (pd.to_numeric(df['facebook_insights_engagements'], errors='coerce') /
                                  pd.to_numeric(df['facebook_insights_impressions'], errors='coerce') * 100).round(2)
        top_threshold = df['engagement_rate'].quantile(0.9) if len(df) else 0
        top_posts = df[df['engagement_rate'] >= top_threshold].copy()
        return {
            "top_posts": top_posts[['facebook_id', 'facebook_post_labels_names', 'engagement_rate', 'facebook_insights_video_views_average_completion', 'facebook_insights_engagements']].to_dict('records'),
        }
    ...
    def generate_worst_performers_analysis(self, df: pd.DataFrame) -> Dict:
        ...
        return {
            "worst_posts": worst_posts[['facebook_id', 'facebook_post_labels_names', 'engagement_rate', 'facebook_insights_video_views_average_completion', 'created_time', 'facebook_insights_engagements']].to_dict('records'),
        }

    def generate_ai_agent_guide(self, overview: Dict, brand_metrics: Dict, content_metrics: Dict, temporal_metrics: Dict) -> Dict:
        return {
            "ai_agent_instructions": {
                "overview": "This metrics suite provides comprehensive Facebook performance analytics for data-driven content strategy",
                "key_files": {
                    "latest_metrics_summary_facebook.json": "Always up-to-date consolidated metrics - START HERE",
                    "facebook_brand_performance_*.json": "Detailed brand analytics and rankings",
                    "facebook_temporal_analytics_*.json": "Posting time optimization data",
                    "facebook_top_performers_*.json": "High-performing content patterns"
                }
            }
        }
    ...
    def generate_consolidated_summary(self, overview: Dict, brand_metrics: Dict, content_metrics: Dict, temporal_metrics: Dict, top_performers: Dict, worst_performers: Dict) -> Dict:
        return {
            ...
            "files_generated": {
                "brand_performance": f"facebook_brand_performance_{self.timestamp}.json",
                "content_performance": f"facebook_content_type_performance_{self.timestamp}.json",
                "temporal_analytics": f"facebook_temporal_analytics_{self.timestamp}.json",
                "top_performers": f"facebook_top_performers_{self.timestamp}.json",
                "worst_performers": f"facebook_worst_performers_{self.timestamp}.json",
                "ai_guide": f"facebook_ai_agent_guide_{self.timestamp}.json"
            }
        }
