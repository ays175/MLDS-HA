#!/usr/bin/env python3
"""
Facebook-specific entity extraction for knowledge graph
"""
import hashlib
import json
import re
...
def extract_facebook_entities(df: pd.DataFrame) -> Dict[str, List[Dict[str, Any]]]:
    """Extract platforms, brands, and content types from Facebook DataFrame."""
...
def generate_facebook_entity_uuids(entities: Dict[str, List[Dict[str, Any]]]) -> Dict[str, Dict[str, str]]:
    """Generate consistent UUIDs for Facebook entities."""
...
def map_facebook_post_relationships(row: pd.Series, entity_uuids: Dict[str, Dict[str, str]]) -> Dict[str, Any]:
    """Map Facebook post to entity relationship UUIDs based on labels."""
    labels = _parse_label_items(row.get('facebook_post_labels_names', ''))
...
def extract_enhanced_text_content(row: pd.Series) -> Dict[str, str]:
...
def extract_facebook_attachments_info(row: pd.Series) -> Dict[str, Any]:
    """Infer basic attachments info from a row.

    Heuristics:
    - If an 'facebook_attachments' JSON-like field exists, count items and infer type
    - Else use 'facebook_media_type' if present
    - Fallback to single photo
    """
    media_type = "photo"
    media_count = 1

    mt = row.get('facebook_media_type') or row.get('media_type')
    if pd.notna(mt):
        try:
    ...
    attachments = row.get('facebook_attachments')
